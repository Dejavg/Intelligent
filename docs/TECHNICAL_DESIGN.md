...
```text
答题卡图片上传 -> 视觉大模型识别整卷内容 -> 题目拆分/学科识别/题型识别 -> 逐题评分 -> 汇总总分/薄弱点/评语
```

该模式使用内置作业“AI 自动识别整张答题卡”，不依赖网站题库中已有题目或标准答案。模型会根据图片中的完整题干与学生作答自行提取评分要点并逐题批改。
...
整张答题卡批改时调用 `LLMClient.grade_answer_sheet`，要求模型输出：

- `questions`：逐题列表，包含题号、学科、题型、题干、学生答案、得分、满分、是否正确、步骤/内容分析、错因、知识点、薄弱点和正确解法；
- `score` / `full_score`：整张答题卡总分与总满分；
- `summary`：整卷分析；
- `comment`：面向学生的整体评语；
- `suggestion`：整体学习建议；
- `common_weak_points`：共性薄弱点。

后端会把该 JSON 归一化到 `grading_results.ai_metadata.answer_sheet`，前端在批改结果页展示“逐题批改”区域。
...
- 数学 7 条：全部正确、第 3 题计算错误、第 4 题方程末步错误、最终答案正确但缺步骤、应用题列式错误、单位缺失、方程移项错误；
- 英语 3 条：一般过去时错误、be 动词缺失、冠词错误。
