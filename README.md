...
- 批改结果页：左侧原始作业图片，右侧总分卡、推荐讲解顺序、题目快速导航和逐题批改卡，重点突出第 3 题计算错误和第 4 题过程分。
- 教师工作台：状态筛选、提交列表、AI 与教师差异、教师复核表单和二次标注闭环说明。
- 班级分析页：仪表盘指标、各题正确率、薄弱知识点排行、高频错误和醒目的 AI 教学建议。
- 管理中心：题库、班级、批量上传、真实 OCR 测试、教师二次标注和评分准确率评测入口。
...
9. 展示逐题批改卡片、知识点薄弱分析和个性化建议。
10. 进入教师工作台。
11. 教师查看 AI 批改结果、AI 原始分数和教师复核分数差异。
12. 教师复核分数和评语，保存复核。
13. 进入班级分析页。
14. 查看平均分、正确率、薄弱点排行、高频错误和 AI 教学建议。
15. 点击“导出报告”，下载班级学情分析 Markdown 文件。
16. 进入管理中心或评测中心。
17. 展示评分准确率评测，说明系统通过样例验证批改稳定性。
18. 如需验证真实能力，在管理中心打开“真实 OCR 测试”，上传真实试卷，只看 OCR 状态、题目数、文本长度和耗时。
19. 结束时总结亮点：整卷识别、逐题过程分、错因定位、教师复核闭环、真实 OCR 可验证入口和班级报告。
