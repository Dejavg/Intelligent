...
阶段 1.5 聚焦比赛收尾、真实能力验证和答辩稳定性：

- 一键演示模式：自动选择张三、固定 5 题数学卷，自动完成上传、OCR、AI 批改和结果跳转。
- 首页运行诊断：显示“当前可安全演示：是 / 否”，并提示真实 OCR 配置是否完整。
- 演示讲解顺序：结果页提供讲解卡和题目快速导航，能快速定位第 3 题、第 4 题。
- 真实 OCR 测试页：管理中心可单独上传真实试卷，只执行 OCR，不影响比赛稳定 Demo。
- 增强评测中心：展示测试样例数、平均分差、评分误差合格率、错因识别准确率、知识点识别准确率、过程分合理率、教师复核一致率和评语完整率。
- 教师逐题复核：教师可逐题查看 AI 分数、错因和知识点，并保存单题复核为标注样本。
- 演示数据重置：`POST /api/demo/reset` 可清空测试提交并恢复默认演示数据。
- 导出能力：结果页和教师详情页支持导出学生个人报告 Markdown 与 AI 批改 JSON。
- 错误状态：统一处理上传中、OCR 中、批改中、详情加载、班级分析加载、API Key 缺失、OCR 配置错误、图片过大、网络失败、后端未启动、无提交记录等提示。

## 阶段 1.6 多图与作文题目优化

本轮优化解决两个更贴近真实教学场景的问题：多页试卷顺序合并批改，以及作文题目/写作要求参与 AI 批改。

- 多图上传：学生上传页支持一次选择多张 `JPG / PNG / JPEG` 图片，按选择顺序生成“第 1 页、第 2 页...”预览。
- 顺序调整：每页图片都支持“上移 / 下移 / 删除”，用于修正题目页、答题页或多页试卷的顺序。
- 多图 OCR：多张图片会走 `/api/upload/batch`、`/api/ocr/batch`、`/api/grade/batch`，先按页 OCR，再按题号合并题干与学生答案，最后统一批改整份作业。
- 分页结果展示：结果页左侧兼容多张原图，展示每页 OCR、合并后的题目与答案、来源页码、合并置信度和警告。
- 作文题目输入：选择“语文 / 英语 + 作文/主观题”时，上传页会显示“作文题目 / 写作要求”输入框。
- 作文 Prompt 增强：后端将 `essay_prompt + OCR 作文正文` 一起传入规则/大模型批改链路，重点判断切题度、内容完整度、结构、语言表达、错别字/语法/拼写和个性化建议。

演示建议：

- 数学整卷可以上传单张完整答题卡，也可以上传多张图片。
- 题目页和答案页分开拍照时，请先上传题目页，再上传答题页；如顺序错误，可用“上移 / 下移”调整。
- 作文批改建议填写作文题目或写作要求；如果留空，结果页会提示“切题判断可能不完整”。

## 页面说明

- 首页：教育科技感产品首屏，展示运行模式、AI 批改预览卡、三步演示流程、核心能力卡片和一键快速演示入口。
- 学生上传页：流程化步骤条、单图/多图拖拽上传卡片、页码预览、顺序调整、作文题目输入、固定 5 题 Demo 说明和“快速演示：使用固定 5 题数学卷”按钮。
- 批改结果页：左侧单张/多张原始作业图片，右侧总分卡、合并识别结果、推荐讲解顺序、题目快速导航、逐题批改卡，并支持导出学生报告 Markdown 和批改 JSON。
- 教师工作台：状态筛选、提交列表、作文题目展示、AI 与教师差异、整份复核、逐题复核表单和二次标注闭环说明。
...
首页模式卡会显示“当前可安全演示”：稳定演示模式为“是”；真实 OCR 模式配置完整时显示“真实 OCR 模式可用”；真实 OCR 模式缺少 Key 或 OCR_PROVIDER 配置异常时会提示检查 API Key 或切回演示模式。
...
| 状态 | 页面提示 | 处理建议 |
| --- | --- | --- |
| 后端未启动 / 网络失败 | “网络失败或后端未启动” | 启动 FastAPI：`D:\anaconda3\envs\env\python.exe -m uvicorn backend.app.main:app --host 127.0.0.1 --port 8000` |
| 没有上传图片 | “请先上传作业图片” | 手动流程需选择图片；比赛可直接点一键演示 |
| 图片过大 | “图片过大” | 压缩到 8MB 以内后重试 |
| API Key 缺失或无效 | “请检查 KIMI_API_KEY / LLM_API_KEY” | 修改 `.env` 后重启后端 |
| OCR_PROVIDER 配置错误 | “请检查 .env” | 使用 `llm`、`paddle`、`baidu`、`tencent` 或回到演示模式 |
| 真实 OCR 模式但 Key 不完整 | “真实 OCR 配置不完整” | 填写 Key 或设置 `DEMO_FIXED_MATH_PAPER_OCR=true` |
| OCR / 批改失败 | 页面 toast 与状态栏显示失败原因 | 查看后端日志，必要时切回稳定演示模式 |
| 没有可用提交记录 | 教师端/结果页空状态提示 | 先运行一键演示或批量上传 Demo |
...
| POST | `/api/upload/batch` | 多图上传，按页保存图片并创建批量提交 |
| POST | `/api/bulk-upload` | 批量创建提交并可自动 OCR/批改 |
| POST | `/api/ocr` | 对提交执行 OCR 识别 |
| POST | `/api/ocr/batch` | 多图按页 OCR，并按题号合并题目与学生作答 |
| POST | `/api/grade` | 对 OCR 文本执行 AI 批改 |
| POST | `/api/grade/batch` | 对多图合并后的整份作业统一逐题批改 |
...
- 多图上传可见性：学生上传页的文件输入已开启 `multiple`，并更新 `index.html` 中的 `app.js` 版本号，避免浏览器继续缓存旧的单图界面。如果仍看到旧页面，请使用 `Ctrl + F5` 强制刷新。
- 上传安全校验：后端对单图和多图上传统一校验真实图片格式，支持 `jpg / jpeg / png`，单张图片限制 `8MB`，批量上传一次最多 `10` 页。
- 多图合并诊断：多图 OCR 合并结果会展示题号、来源页、置信度、合并状态和复核提示，低置信度题目会提示教师复核。
- 真实 OCR 测试增强：管理中心支持“仅 OCR”和“OCR + 合并题目与答案”两种测试方式，展示图片尺寸、耗时、置信度、题目数、合并失败数，并支持下载 OCR 文本和结构化 JSON。
- API Token 管理：管理中心新增本地 `APP_API_TOKEN` 输入框，前端会自动通过 `X-API-Key` 请求头访问受保护接口。
- 演示数据重置更安全：`POST /api/demo/reset` 现在需要提交确认字段 `confirm=RESET_DEMO_DATA`，避免误触清空测试提交。
- 作文评分维度：语文/英语作文结果增加切题度、内容完整、结构层次、语言表达、书写/标点等维度展示，便于答辩解释主观题评分依据。
- 评测中心增强：除原有分数误差、错因识别和知识点识别外，增加多图合并准确率、题号页码匹配率和作文切题判断率展示。
...
## 阶段 1.8 报告、客观题与多图合并增强

本轮优化面向“更好讲清楚真实教学价值”的比赛收尾能力：

- 上传体验：学生上传页支持先上传一批图片后继续追加上传，系统会保留已有页码与顺序，重复图片自动跳过，最多保留 10 页；仍兼容单图上传流程。
- 学生个人报告：`GET /api/students/{id}/report` 新增学生个人得分趋势、知识点掌握度、错题分布和趋势摘要，结果页会用趋势卡片展示最近几次作业表现。
- 班级分析：`GET /api/classes/{class_name}/analysis` 新增班级分层分析和学生对比分析，可区分提升层、稳固层、帮扶层，方便说明系统不只批改，还能辅助教学决策。
- 客观题批改：选择题、填空题、判断题等客观题会优先走规则批改，支持从 OCR 文本中提取“答案：B”“第1空：...”等作答，并给出分数、错因、知识点和客观题匹配明细。
- 多图合并解释性：`/api/ocr/batch` 与 `/api/grade/batch` 的合并结果新增页类型、页级摘要、题目来源页、答案来源页、匹配说明和页角色，能解释题目页和答题页如何对应。

演示建议：

1. 先用固定 5 题数学卷跑稳定演示，展示过程分与错因定位。
2. 再在学生上传页继续追加图片，说明系统支持多页试卷逐步补充上传。
3. 在结果页展示“合并识别结果”的匹配说明，强调题目页和答题页可分开上传。
4. 在班级分析页讲学生分层，在结果页讲学生趋势，让评委看到批改结果如何转化为教学建议。
