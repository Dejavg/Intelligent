...
- 首页：教育科技感产品首屏，展示运行模式、AI 批改预览卡、三步演示流程和核心能力卡片。
- 学生上传页：流程化步骤条、拖拽上传卡片、固定 5 题 Demo 说明。
- 批改结果页：左侧原始作业图片，右侧总分卡和逐题批改卡，重点突出第 3 题计算错误和第 4 题过程分。
- 教师工作台：状态筛选、提交列表、AI 与教师差异、教师复核表单和二次标注闭环说明。
- 班级分析页：仪表盘指标、各题正确率、薄弱知识点排行、高频错误和醒目的 AI 教学建议。
- 管理中心：题库、班级、批量上传、教师二次标注和评分准确率评测入口。
...
1. 稳定演示模式：保持 `DEMO_FIXED_MATH_PAPER_OCR=true`。选择“自动识别 / 答题卡”并上传固定 5 题数学练习卷，系统会返回结构化 OCR 和稳定逐题批改结果：总分 `43 / 50`，第 3 题正确答案为 `62`，第 4 题显示为“部分正确”并得 `7 / 10`。这是比赛现场推荐模式。
...
6. 系统展示总分 `43 / 50`。
7. 展示逐题批改卡片。
8. 重点展示第 3 题计算错误：`90 - 28 = 72`，正确应为 `62`。
9. 重点展示第 4 题过程正确但最终答案错误：前两步正确，最后 `x = 4` 错误，系统给部分分 `7 / 10`。
10. 展示知识点薄弱分析和个性化建议。
11. 进入教师工作台。
12. 教师查看 AI 批改结果、AI 原始分数和教师复核分数差异。
13. 教师复核分数和评语，保存复核。
14. 进入班级分析页。
15. 查看平均分、正确率、薄弱点排行、高频错误和 AI 教学建议。
16. 点击“导出报告”，下载班级学情分析 Markdown 文件。
17. 进入管理中心或评测中心。
18. 展示评分准确率评测，说明系统通过样例验证批改稳定性。
19. 结束时总结亮点：整卷识别、逐题过程分、错因定位、教师复核闭环和班级报告。

## 答辩亮点

- 不是简单判对错，而是能逐题分析过程并给部分分。
- 第 3 题展示计算错误定位，第 4 题展示“过程正确但最终答案错误”的过程分能力。
- 教师复核避免 AI 误判直接影响成绩，复核结果可作为二次标注数据。
